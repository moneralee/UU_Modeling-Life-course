...
def get_dH_surface(x, y, target_sigma, 
                   nx, ny, source_sigma, 
                   sigma_field, tau_field, 
                   sigma_copy, tau_copy, 
                   parameters):
    """
    SURFACE CALCULATION
    The surface (or perimeter in 2D) is calculated as the number of pixels
    that are adjacent to a different cell type or medium.
    """

    def calculate_surface_area(cell_sigma, sigma_field, parameters):
        """
        Helper function that calculates the surface area of a cell at (i, j).

        The standard way to calculate cell perimeter in the CPM is an approximation. 
        For each lattice site with value cell_sigma, count the number of 
        unlike sigma neighbors within the neighborhood.
        The sum of all such neighbors over all lattice sites of cell_sigma is
        the approximated perimeter.

        See:
        Magno, Ramiro, Verônica A. Grieneisen, and Athanasius FM Marée. 
        "The biophysical nature of cells: potential cell behaviours revealed 
        by analytical and computational studies of cell surface mechanics." 
        BMC biophysics 8.1 (2015): 8.
        """
        
        # Use numpy to get indices of all elements that match cell_sigma
        this_cell = np.argwhere(sigma_field == cell_sigma)
        area = 0

        height, width = sigma_field.shape

        for i, j in this_cell:
            for ni, nj in get_neighbors(i, j, width, height, parameters["neighborhood"]):
                neighbor_sigma = sigma_field[ni, nj]
                if neighbor_sigma != cell_sigma:
                    area += 1
        return area

    # Get surface parameters
    surface_ideal  = parameters['surface_ideal']
    surface_weight = parameters['surface_weight']
    ...
    # Initialize variables
    surf_target_energy_before = 0
    surf_target_energy_after  = 0
    surf_source_energy_before = 0
    surf_source_energy_after  = 0

    if target_cell != 0:
        surface_target_before = calculate_surface_area(target_sigma, sigma_field, parameters)
        surface_target_after  = calculate_surface_area(target_sigma, sigma_copy, parameters)
        
        surf_target_energy_before = surface_weight[target_cell - 1] * (surface_target_before - surface_ideal[target_cell - 1])**2
        surf_target_energy_after  = surface_weight[target_cell - 1] * (surface_target_after - surface_ideal[target_cell - 1])**2

    if source_cell != 0:
        surface_source_before = calculate_surface_area(source_sigma, sigma_field, parameters)
        surface_source_after  = calculate_surface_area(source_sigma, sigma_copy, parameters)

        surf_source_energy_before = surface_weight[source_cell - 1] * (surface_source_before - surface_ideal[source_cell - 1])**2
        surf_source_energy_after  = surface_weight[source_cell - 1] * (surface_source_after - surface_ideal[source_cell - 1])**2

    # Step 2
    surface_energy_before = surf_target_energy_before + surf_source_energy_before
    surface_energy_after  = surf_target_energy_after  + surf_source_energy_after

    dH = surface_energy_after - surface_energy_before
...
def calc_dH(x, y, target_sigma, 
    ...
    dH += get_dH_surface(x, y, target_sigma, 
                         nx, ny, source_sigma, 
                         sigma_field, tau_field, 
                         sigma_copy, tau_copy, 
                         parameters)
...
### NEW ###
def is_connectivity_preserved(sigma_copy, target_sigma, x, y, parameters):
    """
    Check if all pixels of 'cell_id' are connected starting from (i, j).
    
    Uses a flood-fill algorithm to determine connectivity:

    1. Start at a seed pixel (i, j). Mark it as visited.
    2. Check neighbours of focal pixel. Remove the focal pixel from the list of pixels to visit.
    3. If a neighbor has the same value (cell ID) and hasn't been visited, add it to the list of pixels to visit.
    4. Repeat until there are no more pixels to visit.
    
    Returns True if the pixels of cell_id are connected, False otherwise.
    """

    # Skip calculation if target is medium or any other special state (sigma < 1).
    if target_sigma < 1: 
        return True

    height, width = sigma_copy.shape
    visited = np.zeros_like(sigma_copy, dtype=bool)  # Track visited pixels
    pixels_to_visit = []
    connected_pixels = 0
    neighborhood = parameters["neighborhood"]
    
    # After a successful copy attempt, the target site no longer belongs to the target sigma.
    # So we first search for a neighbor that has the target sigma.
    # If no such neighbor is found, pixels_to_visit will be empty, and the function returns True. 
    neighbors = get_neighbors(x, y, height, width, neighborhood)
    for nx, ny in neighbors:
        if sigma_copy[nx, ny] == target_sigma:
            pixels_to_visit = [(nx, ny)]
            visited[nx, ny] = True
            connected_pixels += 1
            break # stop looping after finding first neighbor

    while pixels_to_visit:
        i, j = pixels_to_visit.pop(0)  # Take the first pixel in the list
        neighbors = get_neighbors(i, j, height, width, neighborhood)
        for ni, nj in neighbors:
            # If the neighbor is part of the cell and not visited
            if (not visited[ni, nj]) and (sigma_copy[ni, nj] == target_sigma):
                visited[ni, nj] = True
                connected_pixels += 1
                pixels_to_visit.append((ni, nj))  # Add to the list

    # Total pixels of the cell
    total_pixels = np.sum(sigma_copy == target_sigma)
...
def monte_carlo_step_edgelist(sigma_field, tau_field, parameters, 
    ...
    while copy_attempts < total_attempts:
        ...
        ### NEW ###
        # Always reject if copy attempt would disconnect source or target cell
        if not is_connectivity_preserved(sigma_copy, target_sigma, x, y, parameters):
            rejected_copies += 1
            rejected = True
...
if __name__ == "__main__":
    ...
    sim_parameters = {
        ...
        # Surface of the cell (perimeter in 2D)
        'surface_ideal'  : cell_surface,
        'surface_weight' : [1, 1]
    }
